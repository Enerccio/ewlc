...
{
   xcb_destroy_window(x11.connection, bsurface->window);
}
...
update_outputs(struct chck_pool *outputs)
{
   ...
   if ((error = xcb_request_check(x11.connection, xcb_change_window_attributes_checked(x11.connection, x11.screen->root, mask, root_values)))) {
      free(error);

      for (uint32_t i = 0; i < fakes; ++i) {
         xcb_window_t window;
         if (!(window = xcb_generate_id(x11.connection)))
            continue;

         xcb_void_cookie_t create_cookie = xcb_create_window_checked(x11.connection, XCB_COPY_FROM_PARENT, window, x11.screen->root, 0, 0, 800, 480, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT, x11.screen->root_visual, mask, values);
         ...
         xcb_change_property(x11.connection, XCB_PROP_MODE_REPLACE, window, x11.atoms[WM_CLASS], XCB_ATOM_STRING, 8, 7, "wlc-x11");
         xcb_change_property(x11.connection, XCB_PROP_MODE_REPLACE, window, x11.atoms[NET_WM_NAME], x11.atoms[UTF8_STRING], 8, 7, "wlc-x11");
         xcb_change_property(x11.connection, XCB_PROP_MODE_REPLACE, window, x11.atoms[NET_WM_ICON_NAME], x11.atoms[UTF8_STRING], 8, 7, "wlc-x11");
         xcb_change_property(x11.connection, XCB_PROP_MODE_REPLACE, window, XCB_ATOM_WM_CLASS, XCB_ATOM_STRING, 8, 7, "wlc-x11");
         xcb_change_property(x11.connection, XCB_PROP_MODE_REPLACE, window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8, 7, "wlc-x11");
         xcb_change_property(x11.connection, XCB_PROP_MODE_REPLACE, window, XCB_ATOM_WM_ICON_NAME, XCB_ATOM_STRING, 8, 7, "wlc-x11");
         xcb_void_cookie_t map_cookie = xcb_map_window_checked(x11.connection, window);

         if ((error = xcb_request_check(x11.connection, create_cookie)) || (error = xcb_request_check(x11.connection, map_cookie))) {
            free(error);
            continue;
         }
...
setup_xkb(void)
{
   ...
   if (!(ext = xcb_get_extension_data(x11.connection, &xcb_xkb_id)))
      return false;

   x11.xkb_event_base = ext->first_event;

   xcb_void_cookie_t select = xcb_xkb_select_events_checked(x11.connection,
   ...
   xcb_generic_error_t *error;
   if ((error = xcb_request_check(x11.connection, select))) {
      free(error);
      return false;
   }

   xcb_xkb_use_extension_reply_t *use_ext_reply;
   xcb_xkb_use_extension_cookie_t use_ext = xcb_xkb_use_extension(x11.connection, XCB_XKB_MAJOR_VERSION, XCB_XKB_MINOR_VERSION);
   if (!(use_ext_reply = xcb_xkb_use_extension_reply(x11.connection, use_ext, NULL)))
      return false;
   ...
   xcb_xkb_per_client_flags_cookie_t pcf = xcb_xkb_per_client_flags(x11.connection,
                                                                            XCB_XKB_ID_USE_CORE_KBD,
   ...
   if (!(pcf_reply = xcb_xkb_per_client_flags_reply(x11.connection, pcf, NULL)))
      return false;
...
wlc_x11(struct wlc_backend *backend)
{
   ...
   if (!(x11.connection = XGetXCBConnection(x11.display)))
   ...
   for (uint32_t i = 0; i < ATOM_LAST; ++i) {
      xcb_intern_atom_reply_t *reply = xcb_intern_atom_reply(x11.connection, xcb_intern_atom(x11.connection, 0, strlen(map[i].name), map[i].name), 0);
      x11.atoms[map[i].atom] = (reply ? reply->atom : 0);
      free(reply);
   }

   xcb_screen_iterator_t s = xcb_setup_roots_iterator(xcb_get_setup(x11.connection));
   x11.screen = s.data;

   xcb_gc_t gc = xcb_generate_id(x11.connection);
   ...
   xcb_create_pixmap(x11.connection, 1, pixmap, x11.screen->root, 1, 1);
   xcb_create_gc(x11.connection, gc, pixmap, 0, NULL);
   xcb_put_image(x11.connection, XCB_IMAGE_FORMAT_XY_PIXMAP, pixmap, gc, 1, 1, 0, 0, 0, 32, sizeof(data), data);
   xcb_create_cursor(x11.connection, x11.cursor, pixmap, pixmap, 0, 0, 0, 0, 0, 0, 1, 1);
   xcb_free_gc(x11.connection, gc);
   xcb_free_pixmap(x11.connection, pixmap);

   if (!setup_xkb())
      goto could_not_use_xkb_extension;

   if (!(x11.event_source = wl_event_loop_add_fd(wlc_event_loop(), xcb_get_file_descriptor(x11.connection), WL_EVENT_READABLE, x11_event, backend)))
